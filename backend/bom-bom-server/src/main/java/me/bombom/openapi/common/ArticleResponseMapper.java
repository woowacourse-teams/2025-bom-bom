...
public final class ArticleResponseMapper {
    ...
    public static me.bombom.openapi.model.ArticleResponse toApi(
        ...
        newsletter.setImageUrl(domain.newsletter().imageUrl());

        me.bombom.openapi.model.ArticleResponse api = new me.bombom.openapi.model.ArticleResponse(
                domain.articleId(),
                domain.title(),
                domain.contentsSummary(),
                domain.arrivedDateTime().atOffset(ZoneOffset.UTC),
                domain.expectedReadTime(),
                domain.isRead(),
                domain.isBookmarked(),
                newsletter
        );
        api.setThumbnailUrl(domain.thumbnailUrl());
        return api;
    }

...
public class ChallengeTodoReminderNotificationStatusService
        implements NotificationStatusHandler<ChallengeTodoReminderNotification> {

    private final ChallengeTodoReminderNotificationRepository notificationRepository;

    @Override
    @Transactional
    public void updateStatus(ChallengeTodoReminderNotification notification, NotificationResultResponse result) {
        if (result.successCount() > 0) {
            notification.markSent();
            notificationRepository.save(notification);
            log.info("챌린지 TODO 리마인더 발송 완료: notificationId={}, 성공={}, 실패={}, 스킵={}",
        ...
        if (result.skippedCount() == result.totalDevices()) {
            notification.markSent();
            notificationRepository.save(notification);
            log.info("챌린지 TODO 리마인더 모든 기기 스킵: notificationId={}, 스킵={}",
        ...
        notificationRepository.save(notification);
        log.error("챌린지 TODO 리마인더 모든 기기 발송 실패: notificationId={}", notification.getId());
    }

    @Override
    @Transactional
    public void markAsFailed(ChallengeTodoReminderNotification notification, String reason) {
        log.warn("챌린지 TODO 리마인더 FCM 토큰이 없습니다: memberId={}", notification.getMemberId());
